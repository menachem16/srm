function RealTimeNotifications({ user }) {
  ...
  const loadNotifications = async () => {
    try {
      ...
      const notifData = await trickleListObjects(`notifications:${user.objectId}`, 10, true);
      
      if (notifData && notifData.items) {
        setNotifications(notifData.items);
        const unread = notifData.items.filter(n => !n.objectData.isRead).length;
  ...
  try {
    return (
      <div className="relative" data-name="real-time-notifications" data-file="components/RealTimeNotifications.js">
        <button
    ...
    console.error('RealTimeNotifications component error:', error);
    return null;
  }
}